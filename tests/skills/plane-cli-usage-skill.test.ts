...
describe("plane-cli reusable skill", () => {
	it("documents the command surface, plane concepts, and CE constraints", async () => {
		...
		const concepts = await readFile(
			"skills/plane-cli-usage/references/plane-concepts.md",
			"utf8",
		);
		const workflows = await readFile(
			"skills/plane-cli-usage/references/cli-workflows.md",
			"utf8",
		);
		const constraints = await readFile(
			"skills/plane-cli-usage/references/ce-api-constraints.md",
			"utf8",
		);
