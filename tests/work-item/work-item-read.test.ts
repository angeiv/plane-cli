...
describe("work-item read commands", () => {
	...
	beforeEach(async () => {
		configDir = await mkdtemp(
			path.join(os.tmpdir(), "plane-cli-work-item-read-"),
		);
	});
	...
	it("lists work items using per_page and cursor-compatible params", async () => {
		...
		expect(fetchImpl).toHaveBeenCalledWith(
			expect.stringContaining(
				"/api/v1/workspaces/example-workspace/projects/11111111-1111-4111-8111-111111111111/work-items/?per_page=2",
			),
			expect.any(Object),
		);
	});
