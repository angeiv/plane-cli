...
function createWorkItemWriteFetch() {
	return vi.fn(async (input: string | URL, init?: RequestInit) => {
		...
		if (
			url.endsWith("/work-items/22222222-2222-4222-8222-222222222222/") &&
			init?.method === "PATCH"
		) {
			return new Response(
				JSON.stringify({
...
describe("work-item write commands", () => {
	...
	beforeEach(async () => {
		configDir = await mkdtemp(
			path.join(os.tmpdir(), "plane-cli-work-item-write-"),
		);
	});
	...
	it("creates a work item with the minimum name payload", async () => {
		...
		const result = await runCli(
			["work-item", "create", "--name", "CLI test", "--json"],
			{
				configDir,
				fetchImpl,
			},
	...
	it("posts an HTML comment for the resolved work item", async () => {
		...
		expect(result.stdout).toContain(
			'"comment_html": "<p>Temporary comment</p>"',
		);
	});
