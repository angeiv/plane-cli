...
import { resolveFormat, writeFormatted, type OutputFormat } from "../output/format.js";
...
export function createWorkItemCommand(runtime: CliRuntime): Command {
  ...
  workItem
    ...
    .option("--parent <ref>", "Parent work item UUID or sequence ID (use 'none' to clear)")
    ...
    .action(async (options) => {
      try {
        ...
        const ctxOverrides = { projectRef: options.project, workspaceSlug: options.workspace };

        if (options.cycle) {
          await cycleService.addIssues(options.cycle, [String(result.sequence_id)], ctxOverrides);
        }

        if (options.module) {
          await moduleService.addIssues(options.module, [String(result.sequence_id)], ctxOverrides);
        }
        ...
        if (options.cycle) runtime.stdout.write(` (added to cycle ${options.cycle})`);
        if (options.module) runtime.stdout.write(` (added to module ${options.module})`);
  ...
  workItem
    ...
    .action(async (ref: string, options) => {
      try {
        const result = await workItems.listComments(ref, {
          cursor: options.cursor,
  ...
  workItem
    ...
    .action(async (ref: string, commentId: string, options) => {
      try {
        const result = await workItems.updateComment(ref, commentId, options.body, {
          projectRef: options.project,
          workspaceSlug: options.workspace,
        });
