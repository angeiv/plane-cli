...
export function createCycleCommand(runtime: CliRuntime): Command {
  const cycleService = new CycleService(new ConfigStore(runtime.configDir), runtime.fetchImpl);
  const cycle = new Command("cycle").description("Query and mutate Plane cycles");
  ...
  cycle
    ...
    .action(async (ref: string, options) => {
      try {
        ...
        runtime.stdout.write(`Added ${result.length} issue(s) to cycle ${ref}\n`);
      } catch (error) {
        writeError(runtime.stderr, error);
        throw error;
      }
  ...
  cycle
    ...
    .action(async (ref: string, options) => {
      try {
        ...
        runtime.stdout.write(`Removed ${options.issue.length} issue(s) from cycle ${ref}\n`);
      } catch (error) {
        writeError(runtime.stderr, error);
        throw error;
      }
