...
function resolveModuleStatus(value: string): string {
  ...
  if (!resolved) {
    const validChoices = "backlog, planned, in-progress, paused, completed, cancelled";
    throw new Error(`Invalid module status '${value}'. Valid choices: ${validChoices} (or Chinese: 待办, 已计划, 进行中, 已暂停, 已完成, 已取消)`);
  }
  return resolved;
}
...
async function resolveMemberRefs(
  ...
  if (!instance) throw new Error("No active instance. Run `plane auth login` first.");

  const client = new PlaneHttpClient({ apiKey: instance.apiKey, baseUrl: instance.baseUrl, fetchImpl });
  const membersApi = new MembersApi(client);
  const members = await membersApi.list(instance.workspaceSlug ?? "yocloud");

  return refs.map((ref) => {
    if (UUID_RE.test(ref)) return ref;
    const match = members.find((m) => m.email === ref || m.display_name === ref);
    if (!match) throw new Error(`Member '${ref}' not found. Use UUID, email, or display name.`);
    return match.id;
  });
}

export function createModuleCommand(runtime: CliRuntime): Command {
  const moduleService = new ModuleService(new ConfigStore(runtime.configDir), runtime.fetchImpl);
  const module = new Command("module").description("Query and mutate Plane modules");
        ...
        runtime.stdout.write(
          formatTable(
            ...
            [
              ...
              ["lead", result.lead ?? "-"],
              ["members", (result.members && result.members.length > 0) ? result.members.join(", ") : "-"],
            ],
          ),
        );
      } catch (error) {
  ...
  module
    ...
    .option("--status <status>", "Status: backlog/planned/in-progress/paused/completed/cancelled (or Chinese)")
    .option("--start-date <date>", "Start date (YYYY-MM-DD)")
    .option("--target-date <date>", "Target date (YYYY-MM-DD)")
    .option("--lead <ref>", "Lead: UUID, email, or display name")
    .option("--members <ref>", "Member: UUID, email, or display name (repeatable)", collectValues, [])
    .option("--workspace <slug>", "Override workspace slug")
    .option("--project <id-or-key>", "Override project UUID or key")
    .option("--json", "Print JSON output")
    .action(async (options) => {
      try {
        ...
        const resolvedLead = await resolveMemberRefs(new ConfigStore(runtime.configDir), runtime.fetchImpl, leadRefs);
        const resolvedMembers = await resolveMemberRefs(new ConfigStore(runtime.configDir), runtime.fetchImpl, options.members);
        ...
        const result = await moduleService.create({
          ...
          name: options.name,
          status: options.status ? resolveModuleStatus(options.status) : undefined,
  ...
  module
    ...
    .argument("<ref>", "Module UUID or name")
    .requiredOption("--issue <value>", "Work item UUID or sequence ID", collectValues, [])
    .option("--workspace <slug>", "Override workspace slug")
    .option("--project <id-or-key>", "Override project UUID or key")
    .option("--json", "Print JSON output")
    .action(async (ref: string, options) => {
      try {
        ...
        runtime.stdout.write(`Added ${result.length} issue(s) to module ${ref}\n`);
      } catch (error) {
        writeError(runtime.stderr, error);
        throw error;
      }
  ...
  module
    ...
    .action(async (ref: string, options) => {
      try {
        ...
        runtime.stdout.write(`Removed ${options.issue.length} issue(s) from module ${ref}\n`);
      } catch (error) {
        writeError(runtime.stderr, error);
        throw error;
      }
