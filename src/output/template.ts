...
function tokenize(template: string): TemplateToken[] {
  ...
  while ((match = regex.exec(template)) !== null) {
    ...
    } else if (content === "end" && tokens.some((t) => t.type === "range-start")) {
      // end closes the nearest open block — already handled above
...
export function renderTemplate(template: string, data: unknown): string {
  ...
  const renderTokens = (scopeData: unknown, endAt?: "if-end" | "range-end"): void => {
