...
export function resolveFormat(options: {
	...
	if (options.format) {
		...
		throw new Error(
			`Unknown output format '${options.format}'. Valid: table, json, tsv, template, jq`,
		);
	}
	return "table";
}
...
export function writeFormatted(
	...
	switch (format) {
		...
		case "template":
			if (!templateExpr)
				throw new Error(
					"--template requires a template expression, e.g. '{{.name}}'",
				);
		...
		case "jq":
			if (!jqExpr)
				throw new Error(
					"--jq requires a jq expression, e.g. '.results[].name'",
				);
