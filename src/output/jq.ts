...
interface JqNode {
  type: "identity" | "field" | "nested" | "iterate" | "iterate-field" | "object";
  fields?: string[];
...
function parseJq(expr: string): JqNode {
  ...
  const arrayIterMatch = trimmed.match(/^\.(\w+(?:\.\w+)*)\[\](?:\.(\w+(?:\.\w+)*))?$/);
  if (arrayIterMatch) {
    if (arrayIterMatch[2]) {
      return { type: "iterate-field", fields: [...arrayIterMatch[1].split("."), ...arrayIterMatch[2].split(".")] };
    }
...
export function applyJq(expr: string, data: unknown): unknown {
  ...
  switch (node.type) {
    ...
    case "iterate-field": {
      const listPath = node.fields!.slice(0, -1);
