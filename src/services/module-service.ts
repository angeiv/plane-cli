...
export class ModuleService {
  ...
  async list(input: ListModulesInput = {}): Promise<PaginatedResponse<PlaneModule>> {
  ...
  async view(moduleRef: string, overrides: ModuleContextOverrides = {}): Promise<PlaneModule> {
    const { api, workspaceSlug, projectId } = await this.resolveContext(overrides);
    const moduleId = await this.resolveModuleRef(api, moduleRef, workspaceSlug, projectId);

    return api.retrieve(workspaceSlug, projectId, moduleId);
  }

  async create(input: MutateModuleInput & { name: string }): Promise<PlaneModule> {
    const { api, workspaceSlug, projectId } = await this.resolveContext(input);
    const payload = {
  ...
  async update(moduleRef: string, input: MutateModuleInput): Promise<PlaneModule> {
    const { api, workspaceSlug, projectId } = await this.resolveContext(input);
    const moduleId = await this.resolveModuleRef(api, moduleRef, workspaceSlug, projectId);
  ...
  async archive(moduleRef: string, overrides: ModuleContextOverrides = {}): Promise<PlaneModule> {
    const { api, workspaceSlug, projectId } = await this.resolveContext(overrides);
    const moduleId = await this.resolveModuleRef(api, moduleRef, workspaceSlug, projectId);
  ...
  async delete(moduleRef: string, overrides: ModuleContextOverrides = {}): Promise<void> {
    const { api, workspaceSlug, projectId } = await this.resolveContext(overrides);
    const moduleId = await this.resolveModuleRef(api, moduleRef, workspaceSlug, projectId);
  ...
  private async resolveContext(
    overrides: ModuleContextOverrides,
  ): Promise<{ api: ModulesApi; client: PlaneHttpClient; workspaceSlug: string; projectId: string }> {
  ...
  private async resolveModuleRef(
    ...
    if (/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(moduleRef)) {
      return moduleRef;
    }

    const match = await findByPagination(
      (cursor) => api.list(workspaceSlug, projectId, { cursor, perPage: 50 }),
      (item) => item.name.toLowerCase() === moduleRef.toLowerCase() ? item : undefined,
    );

    if (!match) {
      throw new CliError("MODULE_NOT_FOUND", `Module '${moduleRef}' was not found in the active project.`);
    }
