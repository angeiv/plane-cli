...
export class LabelService {
  ...
  private async resolveContext(overrides: LabelContextOverrides) {
    ...
    if (!workspaceSlug) {
      throw new Error("No workspace configured. Run `plane workspace use <slug>` first.");
    }
  ...
  async create(name: string, color?: string, description?: string, overrides: LabelContextOverrides = {}) {
    const { api, workspaceSlug, projectId } = await this.resolveContext(overrides);
  ...
  async delete(labelRef: string, overrides: LabelContextOverrides = {}) {
    ...
    if (!UUID_RE.test(labelRef)) {
      ...
      const match = labels.results.find((l) => l.name.toLowerCase() === labelRef.toLowerCase());
